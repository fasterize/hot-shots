...
const Client = function (
  ...
  this.port =
    options.port || parseInt(process.env.DD_DOGSTATSD_PORT, 10) || 8125;
  ...
  this.globalTags =
    typeof options.globalTags === "object"
  ...
  if (process.env.DD_ENTITY_ID) {
    ...
    this.globalTags.push(
      "dd.internal.entity_id:".concat(
        helpers.sanitizeTags(process.env.DD_ENTITY_ID)
      )
    );
  }
  ...
  this.udsGracefulErrorHandling =
    "udsGracefulErrorHandling" in options
      ? options.udsGracefulErrorHandling
      : true;
  this.udsGracefulRestartRateLimit =
    options.udsGracefulRestartRateLimit || UDS_DEFAULT_GRACEFUL_RESTART_LIMIT; // only recreate once per second
  ...
  if (!options.isChild && this.maxBufferSize > 0) {
    this.intervalHandle = setInterval(
      this.onBufferFlushInterval.bind(this),
 ...
 */
Client.prototype.sendAll = function (
  stat,
  value,
  type,
  sampleRate,
 ...
 * @param callback {Function=} Callback when message is done being delivered. Optional.
 */
Client.prototype.sendStat = function (
  stat,
  value,
  type,
  sampleRate,
...
Client.prototype.send = function (message, tags, callback) {
  ...
  if (mergedTags.length > 0) {
    if (this.telegraf) {
      ...
      message = `${message[0]},${mergedTags
        .join(",")
        .replace(/:/g, "=")}:${message.slice(1).join(":")}`;
    } else {
...
Client.prototype.sendMessage = function (message, callback) {
  ...
  if (socketWasMissing) {
    const error = new Error(
      "Socket not created properly. Check previous errors for details."
    );
    if (callback) {
  ...
  const handleCallback = (err, bytes) => {
    ...
    const errFormatted = err
      ? new Error(`Error sending hot-shots message: ${err}`)
      : null;
    if (errFormatted) {
  ...
  this.flushQueue((err) => {
    ...
    const waitForMessages = setInterval(() => {
      ...
      if (intervalAttempts > 10) {
        console.log(
          "hot-shots could not clear out messages in flight but closing anyways"
        );
        this.messagesInFlight = 0;
      }
...
function udsErrorHandler(client, err) {
  ...
  if (client.errorHandler) {
    ...
    client.socket.on("error", (error) =>
      console.error(`hot-shots UDS error: ${error}`)
    );
  }
}
